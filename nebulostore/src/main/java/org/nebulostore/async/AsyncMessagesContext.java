...
public final class AsyncMessagesContext {
  ...
  /**
   * Remove synchro-group of peer from cache.
   *
   * @param peer
   */
  public synchronized void removeSynchroGroup(CommAddress peer) {
    inboxHoldersMap_.remove(peer);
  }
  ...
  public synchronized void removeUnnecessarySynchroGroups() {
    for (Entry<CommAddress, Set<CommAddress>> entry : inboxHoldersMap_.entrySet()) {
      if (!entry.getValue().contains(myAddress_) && !entry.getKey().equals(myAddress_)) {
        inboxHoldersMap_.remove(entry.getKey());
        recipients_.remove(entry.getKey());
      }
    }
  }
   ...
   *          message to add
   */
  public synchronized void addWaitingAsyncMessage(CommAddress recipient,
